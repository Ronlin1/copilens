...
export default function Dashboard() {
  ...
  const handleDownloadReport = () => {
    ...
    const repo = data.repoInfo?.name || 'repository';
    const now = new Date().toISOString().split('T')[0];

    const report = `COPILENS ANALYSIS REPORT
========================
Repository : ${data.repoInfo?.full_name || repo}
...
Code Files       : ${data.filesChanged}
Lines Added      : ${data.linesAdded?.toLocaleString()}
Lines Deleted    : ${data.linesDeleted?.toLocaleString()}
Stars            : ${data.githubStats?.stars ?? 0}
Forks            : ${data.githubStats?.forks ?? 0}

AI DETECTION
------------
AI-Generated Code : ${data.aiAnalysis?.aiDetection?.percentage ?? 0}%
Confidence        : ${data.aiAnalysis?.aiDetection?.confidence ?? 'N/A'}
AI Commits        : ${data.aiDetectedCommits}
Indicators        : ${(data.aiAnalysis?.aiDetection?.indicators || []).join(', ') || 'None'}

CODE QUALITY
------------
Score         : ${data.aiAnalysis?.codeQuality?.score ?? 'N/A'}/10
Documentation : ${data.aiAnalysis?.codeQuality?.documentation ?? 'N/A'}
Testing       : ${data.aiAnalysis?.codeQuality?.testing ?? 'N/A'}
Strengths     :
${(data.aiAnalysis?.codeQuality?.strengths || []).map(s => `  - ${s}`).join('\n') || '  None'}
Weaknesses    :
${(data.aiAnalysis?.codeQuality?.weaknesses || []).map(w => `  - ${w}`).join('\n') || '  None'}

TECH STACK
----------
Primary      : ${(data.aiAnalysis?.techStack?.primary || []).join(', ') || 'N/A'}
Architecture : ${data.aiAnalysis?.techStack?.architecture ?? 'N/A'}
Frameworks   : ${(data.aiAnalysis?.techStack?.frameworks || []).join(', ') || 'N/A'}
Build Tools  : ${(data.aiAnalysis?.techStack?.buildTools || []).join(', ') || 'N/A'}

LANGUAGES
---------
${(data.languages || []).map(l => `  ${l.name}: ${l.value} bytes`).join('\n') || '  N/A'}

COMPLEXITY
----------
Total Lines        : ${data.complexityData?.totalLines?.toLocaleString() ?? 'N/A'}
Avg Cyclomatic     : ${data.complexityData?.averageCyclomatic ?? 'N/A'}
Avg Cognitive      : ${data.complexityData?.averageCognitive ?? 'N/A'}
Maintainability    : ${data.complexityData?.averageMaintainability ?? 'N/A'}
High-Risk Files    : ${data.complexityData?.topRiskyFiles?.length ?? 0}

TOP RISKY FILES
---------------
${(data.complexityData?.topRiskyFiles || []).slice(0, 10).map((f, i) =>
  `  ${i + 1}. ${f.path} — Risk: ${f.risk?.score ?? 'N/A'}/100 (${f.risk?.level ?? 'N/A'})`
).join('\n') || '  None'}

RECOMMENDATIONS
---------------
${(data.aiAnalysis?.recommendations || []).map((r, i) =>
  `  ${i + 1}. [${r.priority?.toUpperCase() ?? 'N/A'}] ${r.category}: ${r.suggestion}`
).join('\n') || '  None'}

PROJECT HEALTH
--------------
Activity Level    : ${data.aiAnalysis?.projectHealth?.activityLevel ?? 'N/A'}
Maintenance       : ${data.aiAnalysis?.projectHealth?.maintenanceStatus ?? 'N/A'}
Contributor Level : ${data.aiAnalysis?.projectHealth?.contributorActivity ?? 'N/A'}

DEPLOYMENT
----------
Recommended Platform : ${data.recommendedPlatform ?? 'N/A'}

---
Generated by Copilens — AI-Powered Repository Intelligence
https://copilens.up.railway.app
`;
    ...
    a.href = url;
    a.download = `copilens-report-${repo}-${now}.txt`;
    a.click();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-linear-to-r from-cyan-600 to-teal-600 text-white font-semibold rounded-lg hover:from-cyan-700 hover:to-teal-700 transition-all transform hover:scale-105 shadow-lg cursor-pointer"
            >
              <Download size={20} />
              Download Report
            </button>
